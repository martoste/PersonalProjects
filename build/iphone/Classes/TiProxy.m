...
//Rare exceptions to indicate a bug in the _goworkout code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_goworkout = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_goworkout]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.goworkout.module"
